          ...
          txt = (function () {
              ...
              , (function (colors) {
                  ...
                  colors.forEach(function (row) {
                    row.forEach(function (color) {
                      if (color.name in usedRules) {usedRules[color.name] += 1;}
                      else {usedRules[color.name] = 1;}
            ...
            html += ppng.colorNames.map(function (row) {
              ...
              html += row.map(function (cell, idx) {
                var color = cell.rgb.r+", "+cell.rgb.g+", "+cell.rgb.b+", 255";
                var html = "<td class="+cell.name+">";
                html += cell.symbol;
                html += "</td>";
                return html;
              }).join("");
            ...
            html += Object.keys(usedRules).sort(function (k1, k2) {
              return usedRules[k2] - usedRules[k1];
            }).map(function (key) {
              var count = usedRules[key];
              sum += count;
              return "<dt class="+key+">"+key+"</dt><dd>"+count+"</dd>";
            }).join("\n");
